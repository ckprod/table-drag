...
var o={}
for(var n in this.options)o[n]="undefined"===e[n]?this.options[n]:e[n]
...
for(var S=0;S<this.nc;S++){var T=p.cells[S],L=i(T),C=T.offsetWidth,P=T.offsetHeight,M=T.clientWidth,I=T.clientHeight,A=T.clientLeft,B=T.clientTop,O=C-M-A,W=P-I-B,U=r(n(T,"padding-top")),N=r(n(T,"padding-bottom")),k=T.getBoundingClientRect().height-B-W-U-N,R=s?O+A:A,V=s?B+W:B,z=s?O+A:O,H=s?B+W:W,X=s?L.left-g-m:L.left-g,j=s?L.top-y-v:L.top-y,Y=M+R+z,J=h,q=document.createElement("div")
q.style.cssText=f(T),q.style.position="absolute",q.style.left=0,q.style.top=0,q.style.height=k+"px",q.style.borderLeftWidth=R+"px",q.style.borderTopWidth=V+"px",q.style.borderRightWidth=z+"px",q.style.borderBottomWidth=H+"px",q.innerHTML=T.innerHTML,q.style.zIndex=_+3,S==D&&(q.style.left=X+"px"),S==D&&(q.style.top=j+"px")
var F=document.createElement("div")
F.style.position="absolute",F.style.left=X+"px",F.style.top=j+"px",F.style.height=J+"px",F.style.width=Y+"px",F.style.backgroundColor="white",F.style.zIndex=_+2,S==D&&(F.style.zIndex=_+1),S==D&&(this.de=q),S!=D&&F.appendChild(q),E.appendChild(F)}return E.appendChild(this.de),document.body.appendChild(E),this.overlay=E,this.cur=document.body.style.cursor,document.body.style.cursor="move",!0},e.prototype._mouseDrag=function(t){var e=o(t)-o(this._mouseDownEvent),i=this.table,s=this.lc,u=p(i,o(t),s)
